...
export class RestService {
    ...
    private getDefaultConfig(requestMethod, queryParams, queryPrefix, loaderId, config: any = {}): any {
    ...
    private getComputedtedUrl(url: string, pathParams: any): string {
        ...
        result.forEach(match => {
            ...
            // replacing curly braces '?'
            const propNames = propName.replace('?', '');
            if (pathParams[propNames] === undefined) {
                if (!isOptionalParam) {
                    const message = `MPL - Rest Service: ${propNames} is required in pathParam config`;
                    throw new Error(message);
                }
                url = url.replace(match, '');
            } else {
                url = url.replace(match, pathParams[propNames]);
            }
            // replacing trailing '/'
            url = url.replace(/\/$/, '');
        });
